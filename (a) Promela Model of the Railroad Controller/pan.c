...
/*** From source: railroad_controller.pml ***/
...
short src_ln1 [] = {
	  0,  45,  46,  47,  48,  49,  51,  52, 
	 50,  54,  54,  55,  56,  44,  58,  44, 
	 58,   0, };
S_F_MAP src_file1 [] = {
	{ "-", 0, 0 },
	{ "railroad_controller.pml", 1, 16 },
	{ "-", 17, 18 }
};
uchar reached1 [] = {
	  0,   1,   0,   0,   1,   0,   1,   0, 
...
short src_ln0 [] = {
	 ...
	 17,  25,  25,  27,  28,  29,  26,  31, 
	 31,  33,  34,  35,  32,  37,  15,  38, 
	 15,  38,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "railroad_controller.pml", 1, 25 },
	{ "-", 26, 27 }
};
uchar reached0 [] = {
	  0,   1,   1,   0,   0,   1,   0,   0, 
...
{	int j = 0, h = now._nr_pr;
	...
	case 1:	/* Train */
		...
		((P1 *)pptr(h))->req = par0;
		((P1 *)pptr(h))->sig = par1;
		((P1 *)pptr(h))->initialState = par2;
		/* locals: */
		((P1 *)pptr(h))->state = ((P1 *)_this)->initialState;
		((P1 *)pptr(h))->recvd = 0;
#ifdef VAR_RANGES
		logval("Train:initialState", ((P1 *)pptr(h))->initialState);
		logval("Train:state", ((P1 *)pptr(h))->state);
		logval("Train:recvd", ((P1 *)pptr(h))->recvd);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
...
{
		now.requestW = addqueue(calling_pid, 1, 0);
		now.requestE = addqueue(calling_pid, 2, 0);
		now.signalW = addqueue(calling_pid, 3, 0);
		now.signalE = addqueue(calling_pid, 4, 0);
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.requestW) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.requestW) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.requestW) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.requestW) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.requestE) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.requestE) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.requestE) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.requestE) )))
			    ) return 0; break;
		case 3: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.signalW) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.signalW) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.signalW) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.signalW) )))
			    ) return 0; break;
		case 4: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.signalE) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.signalE) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.signalE) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.signalE) )))
			    ) return 0; break;
		case 5: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( ((P1 *)_this)->req) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( ((P1 *)_this)->req) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( ((P1 *)_this)->req) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( ((P1 *)_this)->req) )))
			    ) return 0; break;
		case 6: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( ((P1 *)_this)->sig) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( ((P1 *)_this)->sig) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( ((P1 *)_this)->sig) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( ((P1 *)_this)->sig) )))
			    ) return 0; break;
...
{	/* int i; */
	printf("global vars:\n");
	printf("	mtype  Red:	5\n");
	printf("	mtype  Green:	4\n");
	printf("	mtype  Idle:	3\n");
	printf("	mtype  Waiting:	2\n");
	printf("	mtype  Bridge:	1\n");
	printf("	chan requestW (=%d):	len %d:\t", now.requestW, q_len(now.requestW));
	c_chandump(now.requestW);
	printf("	chan requestE (=%d):	len %d:\t", now.requestE, q_len(now.requestE));
	c_chandump(now.requestE);
	printf("	chan signalW (=%d):	len %d:\t", now.signalW, q_len(now.signalW));
	c_chandump(now.signalW);
	printf("	chan signalE (=%d):	len %d:\t", now.signalE, q_len(now.signalE));
	c_chandump(now.signalE);
	printf("	mtype  ControllerStateW:	%d\n", now.ControllerStateW);
	printf("	mtype  ControllerStateE:	%d\n", now.ControllerStateE);
}
void
...
{	/* int i; */
	...
	printf("	chan req (=%d):	len %d:\t", ((P1 *)pptr(pid))->req, q_len(((P1 *)pptr(pid))->req));
	c_chandump(((P1 *)pptr(pid))->req);
	printf("	chan sig (=%d):	len %d:\t", ((P1 *)pptr(pid))->sig, q_len(((P1 *)pptr(pid))->sig));
	c_chandump(((P1 *)pptr(pid))->sig);
	printf("	mtype  initialState:	%d\n", ((P1 *)pptr(pid))->initialState);
	printf("	mtype  state:	%d\n", ((P1 *)pptr(pid))->state);
	printf("	mtype  recvd:	%d\n", ((P1 *)pptr(pid))->recvd);
		break;
	case 0:
		/* none */
		break;
...
printm(int x, char *s)
{
	...
	switch (x) {
	case 1: Printf("Bridge"); return;
	case 2: Printf("Waiting"); return;
	case 3: Printf("Idle"); return;
	case 4: Printf("Green"); return;
	case 5: Printf("Red"); return;
